/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { OrganizationsWhereInput } from "./OrganizationsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { OrganizationsOrderByInput } from "./OrganizationsOrderByInput";

@ArgsType()
class OrganizationsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => OrganizationsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => OrganizationsWhereInput, { nullable: true })
  @Type(() => OrganizationsWhereInput)
  where?: OrganizationsWhereInput;

  @ApiProperty({
    required: false,
    type: [OrganizationsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [OrganizationsOrderByInput], { nullable: true })
  @Type(() => OrganizationsOrderByInput)
  orderBy?: Array<OrganizationsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { OrganizationsFindManyArgs as OrganizationsFindManyArgs };
